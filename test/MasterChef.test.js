...
contract('MasterChef', ([alice, bob, carol, referrer, treasury, dev, fee, owner]) => {
    beforeEach(async () => {
        ...
        this.referral = await GatorReferral.new({ from: owner });
        this.chef = await MasterChef.new(this.Gator.address, '100', '1000', { from: owner });

        await this.Gator.transferOwnership(this.chef.address, { from: owner });
        await this.referral.updateOperator(this.chef.address, true, { from: owner });
        await this.chef.setGatorReferral(this.referral.address, { from: owner });
